...
export default function RegisterPage(){
  ...
  return (
    <div className="max-w-md mx-auto p-6">
      ...
      {success && <p className="text-sm text-green-600 mb-4">Registration successful! Redirecting to login...</p>}
      {!success && (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <button type="submit" className="bg-primary text-white px-4 py-2 rounded" disabled={loading}>{loading ? 'Processing...' : 'Register'}</button>
          </div>
        </form>
      )}
    </div>
